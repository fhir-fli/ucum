...
class UcumLhcUtils {
        ...
        if (fromUnit != null && toUnit != null) {
          try {
            ...
            if (molecularWeight == null) {
              ...
              toVal = toUnit.convertFrom(Number.double(fromVal), fromUnit);
            } else {
  ...
  ReturnObject convertToBaseUnits(String fromUnit, num fromVal) {
  ...
  ReturnObject getSpecifiedUnit(
    ...
    if (retObj.unit == null) {
      retObj = retObj.copyWith(
          status: retObj.origString == null
              ? ReturnStatus.error
              : ReturnStatus.invalid);
    } else {
      retObj = retObj.copyWith(
          status: retObj.origString == uName
              ? ReturnStatus.valid
              : ReturnStatus.invalid);
    }
    return retObj;
  ...
  ReturnObject validateUnitString(
    String uStr, [
    bool suggest = false,
    String valConv = 'validate',
  ]) {
    var resp = getSpecifiedUnit(uStr, valConv, suggest);
    UcumUnit? theUnit = resp.unit;
    ReturnObject retObj = theUnit == null
        ? ReturnObject.empty()
        : ReturnObject(
    ...
    retObj = retObj.copyWith(
        ...
        msg: resp.retMsg,
        suggestions:
            resp.suggestions != null ? resp.suggestions : retObj.suggestions);
    return retObj;
  }
  ...
  List<dynamic> commensurablesList(String fromName) {
    ...
    final ReturnObject parseResp =
        getSpecifiedUnit(fromName, 'validate', false);
    final UcumUnit? fromUnit = parseResp.unit;
    if ((parseResp.retMsg?.length ?? 0) > 0) {
    ...
    if (fromUnit == null) {
      ...
      List<int>? dimVec;
      Dimension? fromDim = fromUnit.dim_;
      if (fromDim == null) {
