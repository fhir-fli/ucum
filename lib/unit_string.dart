...
class UnitString {
    ...
    if (!endProcessing) {
      ...
      if (!endProcessing) {
        ...
        for (var u1 = 0; u1 < uArray.length; u1++) {
          String? curCode = uArray[u1].un;

          if (UcumUtils.isIntegerUnit(curCode)) {
            uArray[u1].un = int.parse(curCode!).toString();
          } else {
            if (curCode?.contains(parensFlag_) ?? false) {
              ...
              if (!endProcessing) {
                uArray[u1].un = parenUnit.unit.csCode_;
              }
            } else {
              ...
              if (uRet.unit == null) {
                ...
                uArray[u1].un = uRet.unit?.csCode_;
  ...
  ParenProcessing _processParens(String uString, String origString) {
  ...
  ParenProcessing _makeUnitsArray(String uStr, String origString) {
              ...
              if (numRes2 != null &&
                ...
                if (!endProcessing &&
                  ...
                  if (!endProcessing) {
                    ...
                    if (!endProcessing) {
                      uArray.add(
                          Uarray(theOp ?? '', nextParens.unitString ?? ''));
                    }
                  }
                  ...
                  if (parensResp.unitString != null) {
                    ...
                    uArray
                        .add(Uarray(theOp ?? '', parensResp.unitString ?? ''));
                ...
                uArray.add(Uarray(theOp ?? '', uArray1[n] ?? ''));
              }
            } else {
              uArray.add(Uarray(theOp ?? '', uArray1[n] ?? ''));
            }
